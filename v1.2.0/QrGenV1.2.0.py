...
def main():
    try:
        data = get_data(getenv('default_data'))  # Информация для кодировки
        name = get_name(getenv('default_file_name'))  # Название файла
        version = getenv('qr_version')  # Версия QR кода (по умолчанию 1) версии 1-9, версии 10-26 и версии 27-40
        box_size = int(getenv('box_size'))  # (по умолчанию 10)
        border = int(getenv('border'))  # (по умолчанию 4)
        ...
        error_correction = getenv('error_correction').upper()  # уровни восстановления ошибок (L, M, Q, H)
        dep = {
            'L': ERROR_CORRECT_L,
            'M': ERROR_CORRECT_M,
            'Q': ERROR_CORRECT_Q,
